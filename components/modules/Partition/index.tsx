...
const PartitionComponent = ({ enTitle, children, faTitle, id, classNames }: PartitionProps) => {
  return (
    <div id={id} className={cn("w-full mx-auto bg-[#f2f2f0]  lg:rounded-[30px] p-3.5 lg:p-6 flex items-center justify-center", classNames?.wrapper)}>
      <div className="bg-[#f2f2f0] overflow-hidden rounded-lg flex flex-col w-full justify-start h-full gap-2.5 lg:gap-[14px]">
        ...
        {/* BOTTOM SECTION */}
        <div id="BOTTOM__WRAPPER" className={cn("flex flex-col lg:flex-row items-center-safe gap-4 rounded-lg bg-white lg:rounded-[16px]", classNames?.bottom)}>
          {children}
        </div>
      </div>
    </div>
