...
const TabsComponent = () => {
  ...
  return (
    <PartitionComponent
      ...
      faTitle="تب منو"
      id="TAB__MENU"
      classNames={{ wrapper: "h-[634px] sm:h-[750px] lg:h-[627px]", top: "h-[45px] lg:h-[61px]" }}
    >
      <Tabs dir="rtl" defaultValue="1" className="overflow-hidden w-full h-full p-2.5">
        ...
        {tabList.map((item: TabItem) => (
          <TabsContent key={item.id} value={item.id.toString()} className="mt-4 lg:px-16 flex flex-col lg:flex-row gap-3 overflow-hidden">
            {/* TOP SECTION */}
            <div className="xs:p-3 md:p-8 h-1/2 lg:flex lg:flex-col lg:justify-start lg:mt-10 lg:gap-5 lg:basis-1/2">
              <div className="flex items-center-safe font-bold gap-3 text-lg">
                ...
                <h4 className="text-sm md:text-lg font-bold text-nik-foreground">{item.title}</h4>
              </div>
              <div className="font-normal mt-2 text-justify text-nik-foreground leading-6 text-xs md:text-md md:leading-8 lg:leading-8 lg:text-[14px]">
              ...
              <Sheet>
                <SheetTrigger>
                  <Image
                    ...
                    height={200}
                    alt="thumbnail"
                    className="mt-5 xs:mt-2 sm:-mt-5 lg:mt-10"
                  />
                </SheetTrigger>
