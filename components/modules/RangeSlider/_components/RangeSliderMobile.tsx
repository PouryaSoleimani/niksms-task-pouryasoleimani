...
export default function RangeSliderMobile({ min = 10, max = 100, step = 10, defaultValue = 25, onChange }: RangeSliderMobileProps) {
  ...
  return (
    <div className="flex min-w-[350px] flex-row overflow-hidden h-[418px] border-2 m-auto items-center justify-center bg-white p-3" dir="rtl">
        ...
        <div className="flex justify-between -translate-y-3">
          {labels.map((label, idx) => (
            <span key={idx} className="text-[13px] -rotate-90 text-nik-foreground font-medium">
              <span>{label}</span>
              <Minus className="stroke-1 translate-x-1 h-2.5" />
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="mb-12 flex items-center relative inset-0 z-20 bg-transparent backdrop-blur-3xl justify-center translate-x-10 gap-3 font-medium">
        <div id="BLUR__BACKGROUND" className="size-20 bg-radial from-nik-primary via-nik-primary/50 to-white rounded-full absolute top-16 z-0"></div>
        <div className="flex flex-col items-baseline gap-2">
          <span className="text-[24px] text-nik-foreground w-full border center text-center">عدد</span>
          <span className="text-[80px] text-center center w-full font-extrabold text-nik-foreground z-50 backdrop-blur-2xl bg-white/40">{value}</span>
